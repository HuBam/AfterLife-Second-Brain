    ...
    <div className="realm-page">
      ...
      <main className="container mt-xl">
        ...
        <div className="stats-row">
          <div className="stat-card glass-card">
            <Brain size={24} style={{ color: realm.color }} />
            <div>
          ...
          <div className="stat-card glass-card">
            <Target size={24} style={{ color: realm.color }} />
            <div>
              <span className="stat-value">{realm.balance}%</span>
              <span className="stat-label">Balance</span>
            </div>
          </div>
          <Link to="/constellation" className="stat-card glass-card" style={{ textDecoration: 'none', color: 'inherit' }}>
            ...
            >
              ...
              <div className="entry-types">
                {entryTypes.map(({ value, label, icon: Icon }) => (
                  <button
                    key={value}
                    className={`type-btn ${newEntry.type === value ? 'active' : ''}`}
                    onClick={() => setNewEntry({ ...newEntry, type: value })}
                    style={{ '--realm-color': realm.color }}
                  >
                    <Icon size={18} />
                    {label}
              ...
              <textarea
                ...
                className="modal-textarea mt-md"
                rows={4}
              />
        ...
        <section className="entries-section mt-xl">
          ...
          {realm.entries.length === 0 ? (
            <div className="empty-state glass-card">
              <Brain size={48} style={{ color: 'var(--text-muted)', opacity: 0.5 }} />
              <p className="text-muted mt-md">No entries yet</p>
              <p className="text-sm text-secondary">Start logging your skills, knowledge, and insights</p>
            </div>
          ) : (
              ...
              {realm.entries.map((entry, index) => (
                ...
                >
                  <div className="entry-header">
                    <span className="entry-type" style={{ background: realm.color }}>
                      {entry.type}
                    </span>
                    <span className="entry-date text-xs text-muted">
                      {new Date(entry.createdAt).toLocaleDateString()}
                    </span>
                  </div>
                  <h4 className="mt-sm">{entry.title}</h4>
                  {entry.content && (
                    <p className="text-secondary text-sm mt-sm">{entry.content}</p>
                  )}
                </motion.div>
              ))}
            </div>
      ...
      <style>{`
        ...
        .add-entry-btn {
          ...
          border: 1px dashed var(--realm-color);
